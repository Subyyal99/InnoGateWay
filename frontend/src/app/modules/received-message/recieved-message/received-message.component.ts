...
export class ReceivedMessageComponent implements OnInit {
  ...
  tableHeaders = [
    ...
    { field: "error", header: "Error Message", type: "text", edit: "false" },
  ...
  /**
   * table info saves the data to display
   */
  tableInfo = [];
  /**
   *
   ...
   * @param router variable to naigate among pages
   */
  constructor(
  ...
  /**
   * API call to get all sms
   */
  getSms() {
    this.Jarwis.getOthersMessage().subscribe(
  ...
  handleSmsData(data) {
    ...
    data.messageData.forEach((item) => {
      ...
      obj.error = item.recievedSm.errorReceivedSms[0].message;
