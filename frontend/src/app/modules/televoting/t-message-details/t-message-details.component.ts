/**
 * this is TELEVOTING message details component
 */
 ...
 */
@Component({
  selector: "app-t-message-details",
  templateUrl: "./t-message-details.component.html",
  styleUrls: ["./t-message-details.component.css"],
})
export class TMessageDetailsComponent implements OnInit {
  /**
   ...
   * @param messageService is a activated router type variable created to used to retrieve the data in our url if any
   */
  constructor(
  ...
  handleData(data) {
    ...
    data.balanceDeductionMessage.forEach((item) => {
      ...
      obj.status = item.status;
      obj.shortCode = item.shortCode;
      obj.keyword = item.recievedSm.teleVotingCheckKeywords[0].keyword;
