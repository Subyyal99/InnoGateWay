/**
 * this is televoting dashboard component
 */
 ...
 */
@Component({
  selector: "app-televoting-dashboard",
  templateUrl: "./televoting-dashboard.component.html",
  styleUrls: ["./televoting-dashboard.component.css"],
})
export class TelevotingDashboardComponent implements OnInit {
  /**
   ...
   * @param datePipe is a variable of the date pipe library provided by angular
  ...
  /**
   *our function to get short codes by service
   */
  getUserShortCodes() {
   ...
   */
  getAllCounts() {
    this.Jarwis.getAllTelevotingCount().subscribe(
  ...
  getGraphData(dbName, dates?) {
    if (this.showFlag == true && dbName != "Debit") {
      this.Jarwis.getTelevotingGraphData({
    ...
    if (this.showFlag == true && dbName == "Debit") {
      this.Jarwis.getTelevotingDebitGraphData({
        name: dbName,
        shortCode: this.shortCode,
   ...
   */
  route(data) {
    this.router.navigateByUrl("/side-panel/televoting/" + data);
  }
  /**
