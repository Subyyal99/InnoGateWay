 ...
 */
@Component({
  selector: "app-by-msisdn",
  templateUrl: "./by-msisdn.component.html",
  styleUrls: ["./by-msisdn.component.css"],
})
export class ByMsisdnComponent implements OnInit {
  /**
      ...
      [
        {
          ...
          msg: "You must enter Msisdn address",
        },
      ]
    );
  ...
  onSubmit() {
    ...
    this.Jarwis.blackListMsisdnAddress(this.blackListInfo).subscribe(
  ...
  /**
   * API call to get blacklisted msisdn
   */
  getBlackListedMsisdns() {
    this.Jarwis.getBlackListedMsisdns().subscribe(
      (data) => this.handleBlackListedMsisdnsData(data),
      (error) => this.handleError(error)
    );
  }
  /**
   ...
   * @param data is the variable that gets the value from the function call send by the function call
   */
  handleBlackListedMsisdnsData(data) {
    this.tableInfo = [];
   ...
   */
  onDelete(data) {
    this.Jarwis.removeBlackListedMsisdn({ id: data.id }).subscribe(
